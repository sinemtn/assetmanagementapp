      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-b border-gray-200 dark:border-gray-700">
            ...
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Billing Akun</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="(mastercustomer, index) in mastercustomers" :key="mastercustomer.id" class="border-t border-gray-100 dark:border-gray-800">
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ mastercustomer.name }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ mastercustomer.address }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ mastercustomer.billingAccount }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <div class="flex items-center gap-2">

                <!-- Edit Modal -->
                <button type="button" class="text-sm text-gray-700 edit-button" @click="isEditCustomerMaster = true">
                  <EditIcon/>
                  Edit
                </button>
                <Modal v-if="isEditCustomerMaster" @close="isEditCustomerMaster = false">
                  <template #body>
                    <div
                      class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
                      <!-- close btn -->
                      <button @click="isEditCustomerMaster = false"
                        class="transition-color absolute right-5 top-5 z-999 flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:bg-gray-700 dark:bg-white/[0.05] dark:text-gray-400 dark:hover:bg-white/[0.07] dark:hover:text-gray-300">
                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none"
                          xmlns="http://www.w3.org/2000/svg">
                          <path fill-rule="evenodd" clip-rule="evenodd"
                ...
                <!-- Delete Modal -->
                <button type="button" class="text-sm text-gray-700 edit-button" @click="isDeleteCustomerMaster = true">
                  <DeleteIcon/>
                  Delete
                </button>
                <Modal v-if="isDeleteCustomerMaster" @close="isDeleteCustomerMaster = false">
                  <template #body>
                    <div
                    ...
                    >
                      <!-- Close Button -->
                      <button
                        @click="isDeleteCustomerMaster = false"
                      ...
                      <div class="text-center px-2">
                        ...
                        <p class="mb-6 text-sm text-gray-500 dark:text-gray-400">
                         ...
                         <span class="font-semibold text-red-600"> data Customer</span> ini?
                        </p>
                      </div>
...
onMounted(async() => {
  try {
    const response = await axios.get('https://103bec10-ae2f-4484-bfc7-16921f4b1b5e.mock.pstmn.io/api/customer')

    mastercustomers.value = response.data.data
  } catch (error){
...
const isEditCustomerMaster = ref(false);
const EditCustomerMaster = () => {
  console.log('Edit Opened')
  isEditCustomerMaster.value = true
}

const isDeleteCustomerMaster = ref(false);
function confirmDelete() {
  console.log("Data Customer berhasil dihapus!");
  isDeleteCustomerMaster.value = false;
}
