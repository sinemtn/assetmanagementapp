    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex items-center ml-auto gap-2">
        <div
          ...
          <router-link to="/surattugas-detail" class="block">
            <span class="flex items-center gap-2">
              <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none"
                xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path d="M8 1h2v6h6v2H10v6H8V9H2V7h6V1z" fill="black" />
              </svg>
              Tambah
            </span>
          </router-link>
  ...
  <Modal v-if="isAdvancedFilter" @close="isAdvancedFilter = false">
        <template #body>
          <div
            ...
            <!-- close btn -->
            <button @click="isAdvancedFilter = false"
                ...
                <div>
                  <div class="grid grid-cols-1 gap-x-6 gap-y-3 lg:grid-cols-2">
                    ...
                    <div class="col-span-2">
                      ...
                      <input type="text" value="HP LaserJet P3015"
                    ...
                    <div class="col-span-2">
                      ...
                      <input type="text" value="Cyber 2 Tower, Setiabudi"
                        class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" />
                    </div>
                  </div>
    ...
    <div class="max-w-full overflow-x-auto custom-scrollbar">
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-b border-gray-200 dark:border-gray-700">
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Printer ID </p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Manufaktur</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Kategori</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Toner</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Active</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="(masterprinter, index) in masterprinters" :key="masterprinter.printerID" class="border-t border-gray-100 dark:border-gray-800">
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.printerID }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.name }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.manufacture }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.category }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.toner }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ masterprinter.active }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <div class="flex items-center gap-2">

                <router-link to="/stock/printer/edit" class="edit-button">
                  <DeleteIcon />
                  Edit
                </router-link>
                <router-link to="/stock/printer/edit" class="edit-button">
...
const masterprinters = ref([]);
const loading = ref(true);


onMounted(async() => {
  try {
    const response = await axios.get('https://103bec10-ae2f-4484-bfc7-16921f4b1b5e.mock.pstmn.io/api/printer')

    masterprinters.value = response.data.data
  } catch (error){
