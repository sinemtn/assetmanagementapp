    ...
    <div class="max-w-full overflow-x-auto custom-scrollbar">
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-b border-gray-200 dark:border-gray-700">
            ...
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
              <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">MP No.</p>
            </th>
            <th class="px-5 py-3 text-left w-2/11 sm:px-6">
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          <tr v-for="(stockprinter, index) in paginatedData" :key="stockprinter.mpNo"
            class="border-t border-gray-100 dark:border-gray-800">
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ stockprinter.printer }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ stockprinter.mpNo }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ stockprinter.status }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ stockprinter.location }}</p>
            </td>
            <td class="px-5 py-4 sm:px-6">
              <div class="flex items-center gap-2">

                <!-- Edit Modal -->
                <button type="button" class="text-sm text-gray-700 edit-button" @click="isEditStockPrinter = true">
                  <EditIcon />
                  Edit
                </button>
...
onMounted(async () => {
  try {
    const response = await axios.get(
      'https://103bec10-ae2f-4484-bfc7-16921f4b1b5e.mock.pstmn.io/api/stock/printer'
    )
    stockprinters.value = response.data.data
  } catch (error) {
...
const paginatedData = computed(() => {
  ...
  return stockprinters.value.slice(start, start + itemsPerPage.value);
});
